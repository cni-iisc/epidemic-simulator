...
class City:
    ...
    def checkRows(self, df, name="df"):
        if self.nwards is None:
            ...
            assert df.shape[0] == self.nwards, f"Mismatch in {name}: num_rows is not {self.nwards}"
    ...
    def set_demographics(self, inputFiles):
        ...
        demographics = pd.DataFrame.from_dict(inputFiles['demographics']) 
    ...
    def set_employments(self, inputFiles):
        ...
        employments = pd.DataFrame.from_dict(inputFiles['employment'])
    ...
    def set_geoDF(self, inputFiles):
        ...
        print("in set_geoDF")
        city_geojson= json.loads(inputFiles['city'])
        geoDF = gpd.GeoDataFrame.from_features(city_geojson["features"])
        # print(inputFiles['city'])
        # geoDF = inputFiles['city']
    ...
    def set_ODMatrix(self, inputFiles):
        assert self.nwards is not None
        
        try:
            ODMatrix = pd.DataFrame.from_dict(inputFiles['odmatrix'])
    ...
    # Check what to do with this method   
    ...
    def processAgeGivenHH(self, inputFiles):
        ...
        cityprofiledata = inputFiles['city_profile']
    ...
    def set_city_profile(self, inputFiles):
        # assert fileExists(Path(input_dir, inputfiles["cityprofile"])), f"{inputfiles['cityprofile']} missing"
        # with open(Path(input_dir, inputfiles["cityprofile"]),"r") as file:
        #     cityprofiledata = json.load(file)
        cityprofiledata = inputFiles['city_profile']
        ...
        if "ageGivenHouseholdSize" in cityprofiledata.keys():
            self.processAgeGivenHH(inputFiles)
    ...
    def dump_files(self):
        ...
        return self.individuals, self.houses, self.workplaces, self.schools, wardCentreDistances, commonAreas, fractionPopulations
    ...
    def __init__(self, inputFiles, random_seed_dir = None):
        self.reset()
        print("in __init__ of city gen")
        self.set_city_profile(inputFiles)
        ...
        self.set_demographics(inputFiles)
        self.set_employments(inputFiles)
        self.set_ODMatrix(inputFiles)
        print("CityGen_upto_ODMatrix")
        self.set_geoDF(inputFiles) 
