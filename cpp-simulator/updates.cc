...
//Returns whether the node was infected or turned symptomatic in this time step
// CORRECTED update_infection function for updates.cc
// This matches the JavaScript reference implementation exactly
...
node_update_status update_infection(agent& node, int cur_time){
    ...
    // PRE_SYMPTOMATIC → SYMPTOMATIC or RECOVERED
        ...
        if(transition){
            ...
            GLOBAL.daily_new_symptomatic++;
        }
        else {
            // Asymptomatic - recover without symptoms
            node.state_before_recovery = node.infection_status;
            node.infection_status = Progression::recovered;
            node.infective = false;
            GLOBAL.daily_new_recovered++;
        }
    }
    // SYMPTOMATIC → HOSPITALISED or RECOVERED (with detection logic)
    else if(node.infection_status == Progression::symptomatic
            && (double(cur_time) - node.time_of_infection 
                >= (node.incubation_period + node.asymptomatic_period + node.symptomatic_period))){
        
        // DETECTION LOGIC - happens ONCE here, matching JavaScript
        bool detected_during_symptomatic = false;
        if (!node.test_status.tested_positive) {
            detected_during_symptomatic = bernoulli(GLOBAL.case_infection_ratio);
            if (detected_during_symptomatic) {
                node.test_status.tested_positive = true;
                GLOBAL.num_detected_cases++;
                GLOBAL.daily_new_detected++;
            }
        }
        
        // HOSPITALIZATION LOGIC - same probability for both detected and undetected
        ...
        if(transition){
            ...
            GLOBAL.daily_new_hospitalised++;
            
            // If not detected during symptomatic phase, detect now due to hospitalization
            if (!node.test_status.tested_positive) {
                node.test_status.tested_positive = true;
                GLOBAL.num_detected_cases++;
                GLOBAL.daily_new_detected++;
    ...
    // Update lambda values (same as before)
...
#pragma omp parallel for default(none) shared(nodes, SIZE, MASK_FACTOR, cur_time) \
...
//TODO: add cohorts here!!
void update_lambdas(agent&node, const vector<house>& homes, const vector<workplace>& workplaces, const vector<community>& communities, const vector<vector<nbr_cell>>& nbr_cells, const double travel_fraction, const int cur_time, std::unordered_map<count_type, vector<cohort_space>>& cohorts){
  ...
  //Updating the lambdas for agents in cohorts
...
#pragma omp parallel for default(none) shared(nodes, community, SIZE) reduction (+: sum_value)
  for(count_type i = 0; i < SIZE; ++i){
	sum_value
...
#pragma omp parallel for default(none) shared(houses, nodes, HOUSES_SIZE)
...
#pragma omp parallel for default(none) shared(houses, communities, HOUSES_SIZE)
  for(count_type i = 0; i < HOUSES_SIZE; ++i){
...
void reset_daily_counters() {
    GLOBAL.daily_new_exposed = 0;
    GLOBAL.daily_new_presymptomatic = 0;
    GLOBAL.daily_new_symptomatic = 0;
    GLOBAL.daily_new_hospitalised = 0;
    GLOBAL.daily_new_critical = 0;
    GLOBAL.daily_new_recovered = 0;
    GLOBAL.daily_new_deaths = 0;
    GLOBAL.daily_new_detected = 0;
}

// Update cumulative counters at the end of each day
void update_cumulative_counters() {
    GLOBAL.cumulative_new_exposed += GLOBAL.daily_new_exposed;
    GLOBAL.cumulative_new_presymptomatic += GLOBAL.daily_new_presymptomatic;
    GLOBAL.cumulative_new_symptomatic += GLOBAL.daily_new_symptomatic;
    GLOBAL.cumulative_new_hospitalised += GLOBAL.daily_new_hospitalised;
    GLOBAL.cumulative_new_critical += GLOBAL.daily_new_critical;
    GLOBAL.cumulative_new_recovered += GLOBAL.daily_new_recovered;
    GLOBAL.cumulative_new_deaths += GLOBAL.daily_new_deaths;
    GLOBAL.cumulative_new_detected += GLOBAL.daily_new_detected;
}