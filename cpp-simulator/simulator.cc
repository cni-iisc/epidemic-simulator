...
plot_data_struct run_simulation()
{
	...
	plot_data.daily_cumulative_stats =
		{
			{"daily_new_exposed", {}},
	...
	for (auto &elem : plot_data.daily_cumulative_stats)
	{
		elem.second.reserve(GLOBAL.NUM_TIMESTEPS);
	}	
...
#pragma omp parallel for firstprivate(NUM_PEOPLE) default(none) shared(nodes, GLOBAL)          \
	reduction(+                                                                   \
		...
		// Add daily and cumulative tracking data
		plot_data.daily_cumulative_stats["daily_new_exposed"].push_back({time_step, {GLOBAL.daily_new_exposed}});
		plot_data.daily_cumulative_stats["daily_new_presymptomatic"].push_back({time_step, {GLOBAL.daily_new_presymptomatic}});
		plot_data.daily_cumulative_stats["daily_new_symptomatic"].push_back({time_step, {GLOBAL.daily_new_symptomatic}});
		plot_data.daily_cumulative_stats["daily_new_hospitalised"].push_back({time_step, {GLOBAL.daily_new_hospitalised}});
		plot_data.daily_cumulative_stats["daily_new_critical"].push_back({time_step, {GLOBAL.daily_new_critical}});
		plot_data.daily_cumulative_stats["daily_new_recovered"].push_back({time_step, {GLOBAL.daily_new_recovered}});
		plot_data.daily_cumulative_stats["daily_new_deaths"].push_back({time_step, {GLOBAL.daily_new_deaths}});
		plot_data.daily_cumulative_stats["daily_new_detected"].push_back({time_step, {GLOBAL.daily_new_detected}});
		plot_data.daily_cumulative_stats["cumulative_new_exposed"].push_back({time_step, {GLOBAL.cumulative_new_exposed}});
		plot_data.daily_cumulative_stats["cumulative_new_presymptomatic"].push_back({time_step, {GLOBAL.cumulative_new_presymptomatic}});
		plot_data.daily_cumulative_stats["cumulative_new_symptomatic"].push_back({time_step, {GLOBAL.cumulative_new_symptomatic}});
		plot_data.daily_cumulative_stats["cumulative_new_hospitalised"].push_back({time_step, {GLOBAL.cumulative_new_hospitalised}});
		plot_data.daily_cumulative_stats["cumulative_new_critical"].push_back({time_step, {GLOBAL.cumulative_new_critical}});
		plot_data.daily_cumulative_stats["cumulative_new_recovered"].push_back({time_step, {GLOBAL.cumulative_new_recovered}});
		plot_data.daily_cumulative_stats["cumulative_new_deaths"].push_back({time_step, {GLOBAL.cumulative_new_deaths}});
		plot_data.daily_cumulative_stats["cumulative_new_detected"].push_back({time_step, {GLOBAL.cumulative_new_detected}});
		plot_data.daily_cumulative_stats["total_detected_cases"].push_back({time_step, {GLOBAL.num_detected_cases}});
		...
		// update cumulative counters at the end of each day
			if ((time_step + 1) % GLOBAL.SIM_STEPS_PER_DAY == 0)
			{
