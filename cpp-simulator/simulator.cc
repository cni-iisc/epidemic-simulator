...
template <class T>
auto duration(
    const std::chrono::time_point<T>& start, const std::chrono::time_point<T>& end)
{
...
plot_data_struct run_simulation()
{
    ...
    svd home_age_matrix, school_age_matrix, workplace_age_matrix, community_age_matrix;
    ...
    plot_data.nums = {{"num_infected", {}}, {"num_exposed", {}}, {"num_hospitalised", {}},
        {"num_symptomatic", {}}, {"num_critical", {}}, {"num_fatalities", {}},
        {"num_recovered", {}}, {"num_affected", {}}, {"num_cases", {}},
        {"num_cumulative_hospitalizations", {}}, {"num_cumulative_infective", {}}};
    ...
    plot_data.susceptible_lambdas = {{"susceptible_lambda", {}},
        {"susceptible_lambda_H", {}}, {"susceptible_lambda_W", {}},
        {"susceptible_lambda_C", {}}, {"susceptible_lambda_T", {}}};

    plot_data.total_lambda_fractions = {{"total_fraction_lambda_H", {}},
        {"total_fraction_lambda_W", {}}, {"total_fraction_lambda_C", {}},
        {"total_fraction_lambda_T", {}}};

    plot_data.mean_lambda_fractions = {{"mean_fraction_lambda_H", {}},
        {"mean_fraction_lambda_W", {}}, {"mean_fraction_lambda_C", {}},
    ...
    plot_data.quarantined_stats = {{"quarantined_stats", {}}, {"curtailment_stats", {}}};
    ...
    cerr << "simulator: time for setup after JSON reads (ms): "
    ...
    count_type num_cases =
        0;  // Total number of agents who have progessed to symptomatic so far
    count_type quarantined_num_cases = 0;
    count_type num_cumulative_hospitalizations =
        0;  // Total number of agents who have had to go to the hospital so far
    count_type num_cumulative_infective = 0;  // Total number of people who have
                                              // progressed to the infective state so far

    count_type num_total_infections = 0;
    // Total number of individuals who have become infected via transmission so
    // far This does not included the initially seeded infections

    std::vector<long double> infections_by_new_infectives(GLOBAL.NUM_TIMESTEPS, 0);
    // For keeping track of infections ascribed to agents that became infective
    // at each time
    ...
    for(count_type time_step = 0; time_step < GLOBAL.NUM_TIMESTEPS; ++time_step) {
        ...
        if(time_step % GLOBAL.SIM_STEPS_PER_DAY == 0) {
            for(count_type j = 0; j < GLOBAL.num_people; ++j) {
                ...
                nodes[j].forced_to_take_train =
                    GLOBAL.TRAINS_RUNNING &&
        ...
        for(count_type j = 0; j < NUM_PEOPLE; ++j) {
            ...
            if(node_update_status.new_infection) {
                ...
                // update the mean fractions with the new data point
                // normalized_lambda

                {
                    total_lambda_fraction_data += nodes[j].lambda_incoming;
                    auto normalized_lambda = (nodes[j].lambda_incoming / nodes[j].lambda);
                    mean_lambda_fraction_data.mean_update(
                        normalized_lambda, num_new_infections);
                    cumulative_mean_lambda_fraction_data.mean_update(
                        normalized_lambda, num_total_infections);
                }
            }
        ...
        update_all_kappa(
            nodes, homes, workplaces, communities, nbr_cells, intv_params, time_step);

        if(GLOBAL.USE_AGE_DEPENDENT_MIXING) {
            for(count_type h = 0; h < GLOBAL.num_homes; ++h) {
                homes[h].age_dependent_mixing =
                    updated_lambda_h_age_dependent(nodes, homes[h], home_age_matrix.u,
                        home_age_matrix.sigma, home_age_matrix.vT);
            }
            for(count_type w = 0; w < GLOBAL.num_schools + GLOBAL.num_workplaces; ++w) {
                if(workplaces[w].workplace_type == WorkplaceType::school) {
                    workplaces[w].age_dependent_mixing = updated_lambda_w_age_dependent(
                        nodes, workplaces[w], school_age_matrix.u,
                        school_age_matrix.sigma, school_age_matrix.vT);
                }
                else {
                    workplaces[w].age_dependent_mixing = updated_lambda_w_age_dependent(
                        nodes, workplaces[w], workplace_age_matrix.u,
                        workplace_age_matrix.sigma, workplace_age_matrix.vT);
                }
            }
        }
        else {
            for(count_type h = 0; h < GLOBAL.num_homes; ++h) {
                homes[h].age_independent_mixing =
                    updated_lambda_h_age_independent(nodes, homes[h]);
                // FEATURE_PROPOSAL: make the mixing dependent on
                // node.age_group;
            }

            for(count_type w = 0; w < GLOBAL.num_schools + GLOBAL.num_workplaces; ++w) {
                workplaces[w].age_independent_mixing =
        ...
        for(count_type c = 0; c < GLOBAL.num_communities; ++c) {
            ...
            // let row =
            // [time_step/SIM_STEPS_PER_DAY,c,temp_stats[0],temp_stats[1],temp_stats[2],temp_stats[3],temp_stats[4]].join(",");
            plot_data.nums["csvContent"].push_back({time_step,
                {c, temp_stats.affected, temp_stats.susceptible, temp_stats.exposed,
                    temp_stats.infective, temp_stats.symptomatic, temp_stats.hospitalised,
                    temp_stats.critical, temp_stats.dead, temp_stats.recovered,
                    temp_stats.recovered_from_infective,
                    temp_stats.recovered_from_symptomatic,
                    temp_stats.recovered_from_hospitalised,
                    temp_stats.recovered_from_critical, temp_stats.hd_area_affected,
                    temp_stats.hd_area_susceptible, temp_stats.hd_area_exposed,
                    temp_stats.hd_area_infective, temp_stats.hd_area_symptomatic,
                    temp_stats.hd_area_hospitalised, temp_stats.hd_area_critical,
                    temp_stats.hd_area_dead, temp_stats.hd_area_recovered,
            ...
            communities[c].lambda_community =
...
#pragma omp parallel for default(none) \
    shared(travel_fraction, time_step, homes, workplaces, communities, nodes)
        for(count_type j = 0; j < NUM_PEOPLE; ++j) {
            update_lambdas(
                nodes[j], homes, workplaces, communities, travel_fraction, time_step);
        }

        // Get data for this simulation step
        count_type n_infected = 0, n_exposed = 0, n_hospitalised = 0, n_symptomatic = 0,
                   n_critical = 0, n_fatalities = 0, n_recovered = 0, n_affected = 0,
                   n_infective = 0, quarantined_individuals = 0,
                   quarantined_infectious = 0;

        double susceptible_lambda = 0, susceptible_lambda_H = 0, susceptible_lambda_W = 0,
               susceptible_lambda_C = 0, susceptible_lambda_T = 0;
...
#pragma omp parallel for default(none) shared(nodes, GLOBAL)			\
  reduction(+: n_infected, n_exposed,									\
        ...
        for(count_type j = 0; j < NUM_PEOPLE; ++j) {
            ...
            if(infection_status == Progression::infective ||
                ...
                curtailed_interaction +=
                    (nodes[j].kappa_H_incoming * GLOBAL.BETA_H +
                        nodes[j].kappa_C_incoming * GLOBAL.BETA_C +
                        ((nodes[j].workplace_type == WorkplaceType::office)
                                ? GLOBAL.BETA_W
                                : 0) *
                            nodes[j].kappa_W_incoming +
                        ((nodes[j].workplace_type == WorkplaceType::school)
                                ? GLOBAL.BETA_S
                                : 0) *
                            nodes[j].kappa_W_incoming +
            ...
            if(nodes[j].quarantined &&
                (infection_status == Progression::infective ||
                    infection_status == Progression::symptomatic ||
                    infection_status == Progression::hospitalised ||
                    infection_status == Progression::critical)) {
        ...
        plot_data.nums["num_cumulative_hospitalizations"].push_back(
            {time_step, {num_cumulative_hospitalizations}});
        plot_data.nums["num_cumulative_infective"].push_back(
            {time_step, {num_cumulative_infective}});

        plot_data.susceptible_lambdas["susceptible_lambda"].push_back(
            {time_step, {susceptible_lambda}});
        plot_data.susceptible_lambdas["susceptible_lambda_H"].push_back(
            {time_step, {susceptible_lambda_H}});
        plot_data.susceptible_lambdas["susceptible_lambda_W"].push_back(
            {time_step, {susceptible_lambda_W}});
        plot_data.susceptible_lambdas["susceptible_lambda_C"].push_back(
            {time_step, {susceptible_lambda_C}});
        plot_data.susceptible_lambdas["susceptible_lambda_T"].push_back(
            {time_step, {susceptible_lambda_T}});
        ...
        plot_data.total_lambda_fractions["total_fraction_lambda_H"].push_back(
            {time_step, {total_lambda_fraction_data.home}});
        plot_data.total_lambda_fractions["total_fraction_lambda_W"].push_back(
            {time_step, {total_lambda_fraction_data.work}});
        plot_data.total_lambda_fractions["total_fraction_lambda_C"].push_back(
            {time_step, {total_lambda_fraction_data.community}});
        plot_data.total_lambda_fractions["total_fraction_lambda_T"].push_back(
            {time_step, {total_lambda_fraction_data.travel}});

        plot_data.mean_lambda_fractions["mean_fraction_lambda_H"].push_back(
            {time_step, {mean_lambda_fraction_data.home}});
        plot_data.mean_lambda_fractions["mean_fraction_lambda_W"].push_back(
            {time_step, {mean_lambda_fraction_data.work}});
        plot_data.mean_lambda_fractions["mean_fraction_lambda_C"].push_back(
            {time_step, {mean_lambda_fraction_data.community}});
        plot_data.mean_lambda_fractions["mean_fraction_lambda_T"].push_back(
            {time_step, {mean_lambda_fraction_data.travel}});

        plot_data.cumulative_mean_lambda_fractions["cumulative_mean_fraction_lambda_H"]
            .push_back({time_step, {cumulative_mean_lambda_fraction_data.home}});
        plot_data.cumulative_mean_lambda_fractions["cumulative_mean_fraction_lambda_W"]
            .push_back({time_step, {cumulative_mean_lambda_fraction_data.work}});
        plot_data.cumulative_mean_lambda_fractions["cumulative_mean_fraction_lambda_C"]
            .push_back({time_step, {cumulative_mean_lambda_fraction_data.community}});
        plot_data.cumulative_mean_lambda_fractions["cumulative_mean_fraction_lambda_T"]
            .push_back({time_step, {cumulative_mean_lambda_fraction_data.travel}});
        plot_data.quarantined_stats["quarantined_stats"].push_back({time_step,
            {quarantined_individuals, quarantined_infectious, quarantined_num_cases}});
        plot_data.curtailment_stats["curtailment_stats"].push_back(
            {time_step, {normal_interaction, curtailed_interaction}});
    }

    // Create CSV data out of the date for infections per new infective node
    plot_data.infections_by_new_infectives = {{"infections_by_new_infectives", {}}};
    for(count_type time_step = 0; time_step < GLOBAL.NUM_TIMESTEPS; ++time_step) {
        plot_data.infections_by_new_infectives["infections_by_new_infectives"].push_back(
            {time_step, {infections_by_new_infectives[time_step]}});
    }
